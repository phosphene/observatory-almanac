...
**Maximum single dart score:** Triple 20 = 60 points.
**Maximum three-dart score:** Three triple-20s = 180 points ("maximum").
...
**Standard 501:**
- Players start at 501 (sometimes 301 in shorter formats).
- Each player throws **three darts per turn**.
- Score is subtracted from the running total after each turn.
- A player wins by reaching **exactly zero**, with the **final dart in a double** (or bull).
...
If a player's remaining score would go below zero, or reach exactly 1 (which is impossible to finish since no single-point double exists), or reach zero without a double, the turn is **busted**:
...
To close a number, a player must accumulate **3 marks** on it. A mark is:
- A single hit = 1 mark
...
Once a player has closed a number, every additional dart landing there scores points at face value:
- Closed 20: Single = 20 pts, Double = 40 pts, Triple = 60 pts.
- Bull: Outer bull (25) = 25 pts, Inner bull = 50 pts.
...
The first player to:
1. Close all seven numbers (20 through 15 and bull), AND
2. Have a score equal to or greater than the opponent's score,
...
1. Players throw in turn, each throwing three darts.
2. You must hit your current target number before advancing to the next.
3. Hitting a double counts as **two numbers** advanced (e.g., hitting double 7 when targeting 7 means you advance to 9). Hitting a triple advances **three numbers**.
4. The first player to hit 20 (and the bull, if included) wins.

### Variations

**Strict single:** Only a single counts. No doubling/tripling ahead.
**Cutthroat/Reverse:** Hitting a double sends your opponent **back** two numbers.
**Around and Back:** Complete 1 to 20 and then return from 20 to 1.
