...
**The Technique:**
1. Sit comfortably. Use your right hand: place your index and middle fingers between your eyebrows. Thumb will close the right nostril; ring finger will close the left.
2. Close the right nostril with the thumb. Inhale through the left nostril for 4 counts.
3. Close both nostrils. Hold for 4 counts (or longer with practice).
4. Open the right nostril. Exhale through the right for 8 counts.
...
## 🌬️ Box Breathing Timer

<div id="breath-tool" markdown>

<div style="text-align:center; padding:20px; background:#f5f5f0; border-radius:8px; margin:1em 0;">
<div style="position:relative; width:200px; height:200px; margin:0 auto;">
<svg width="200" height="200">
<circle id="breath-circle" cx="100" cy="100" r="80" fill="none" stroke="#4a9" stroke-width="6"
  stroke-dasharray="502" stroke-dashoffset="0" style="transition: stroke-dashoffset 1s linear;"/>
</svg>
<div id="breath-phase" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); font-size:1.3em; font-weight:bold;">Ready</div>
</div>
<div id="breath-count" style="font-size:2.5em; font-family:monospace; margin:12px 0;">4</div>
<div id="breath-cycles" style="color:#666; margin:8px 0;">Cycles: 0</div>
<div style="margin:12px 0;">
<button onclick="breathStart()" style="padding:10px 20px; font-size:16px; margin:4px; cursor:pointer; border-radius:6px; border:1px solid #888; background:#fff;">▶ Start</button>
<button onclick="breathStop()" style="padding:10px 20px; font-size:16px; margin:4px; cursor:pointer; border-radius:6px; border:1px solid #888; background:#fff;">⏹ Stop</button>
</div>
</div>
...
(function() {
  ...
  let timer = null, phaseIdx = 0, count = 4, cycles = 0;

  function update() {
    document.getElementById("breath-phase").textContent = phases[phaseIdx];
    document.getElementById("breath-count").textContent = count;
    document.getElementById("breath-cycles").textContent = "Cycles: " + cycles;
    const progress = (4 - count) / 4;
    const offset = CIRC * (1 - (phaseIdx + progress) / 4);
    document.getElementById("breath-circle").setAttribute("stroke-dashoffset", offset);
    const colors = ["#4a9","#49a","#a94","#94a"];
    document.getElementById("breath-circle").setAttribute("stroke", colors[phaseIdx]);
  }

  window.breathStart = function() {
    if (timer) return;
    phaseIdx = 0; count = 4; cycles = 0;
    update();
    timer = setInterval(() => {
      count--;
      if (count <= 0) {
        phaseIdx++;
        if (phaseIdx >= 4) { phaseIdx = 0; cycles++; }
        count = 4;
      }
      update();
    }, 1000);
