...
*The Observatory Almanac â Language & Communication*

Before telephones and radio, humans developed ingenious systems to communicate across distance and noise. Today these codes still matter â in aviation, maritime emergencies, amateur radio, accessibility design, and situations where voice communication fails. This chapter covers six essential systems: the NATO phonetic alphabet, Morse code, Braille, semaphore, international maritime signal flags, and binary encoding.
...
**When to use it:**
- Spelling callsigns, names, or codes over radio
- Confirming flight numbers, passport numbers, registration plates
- Any situation where a mishearing could cause problems

**Basic structure:**
- "My name is Baker â that's Bravo, Alpha, Kilo, Echo, Romeo"
- "Flight number Juliet Alpha Four Five Niner"

**Aviation example:**
When a pilot checks in with air traffic control: "London Center, this is Speedbird Two Sierra Foxtrot, flight level three five zero."

**Military/police example:**
"Suspect plate is Oscar Mike Victor three four six."

**Everyday use:**
Useful when spelling email addresses, confirmation codes, or anything over a noisy phone connection. "My email is j-a-v-e-s â that's Juliet, Alpha, Victor, Echo, Sierra â at gmail dot com."
...
Morse code uses two signals â a short dot (Â·) and a longer dash (â) â to represent letters, numbers, and punctuation. Timing is everything: dots and dashes within a letter are separated by short gaps; letters within a word by medium gaps; words by long gaps.
...
Transmitted as a continuous sequence without gaps between letters. It was chosen because it's easy to recognize and hard to mistake. It does not stand for "Save Our Souls" or "Save Our Ship" â those are retroactive folk etymologies. It was adopted in 1906 purely for its simplicity.
...
| WATER | Â· â â / Â· â / â / Â· / â Â· â Â· |
| FOOD | Â· Â· â Â· / â â â / â â â / â Â· Â· |
| OK | â â â / â Â· â |
| STOP | Â· Â· Â· / â / â â â / Â· â â Â· |

### How to Signal Morse

**By sound (key or beeper):**
- Dot = short tone (1 unit)
- Dash = long tone (3 units)
...
- Short flash = dot
- Long flash = dash
- A flashlight, mirror, or even blinking can work
- Usable at night over long distances with even a small light

**By knock:**
- Single knock = dot
- Triple knock = dash (or a held knock)
- Usable through walls, pipes, locked doors

**By visual:**
- Waving a cloth: short wave = dot, long wave = dash
- Useful on open water or high ground
...
Braille was developed by Louis Braille, a French educator who was blind, published in 1829. It is a tactile writing system using raised dots arranged in a 2Ã3 cell (two columns, three rows) giving 64 possible combinations including the empty cell.

### The Braille Cell

```
â â    (positions 1 and 4 â top row)
â â    (positions 2 and 5 â middle row)
â â    (positions 3 and 6 â bottom row)
```
...
### Grade 1 Braille â The Alphabet
...
Numbers in Braille use the same cell patterns as AâJ but are preceded by a number indicator symbol (dots 3,4,5,6):
...
**Grade 1 (uncontracted):** Each cell represents one letter or symbol â equivalent to basic literacy. Used for people learning Braille.
...
Braille readers move their fingertips lightly across lines of raised dots. Skilled readers can read at 200+ words per minute. The key is lightness of touch â pressing too hard reduces sensitivity.
...
Letters use combinations of 8 positions: the 4 cardinal directions + 4 diagonal directions (45Â°, 135Â°, 225Â°, 315Â°).
...
**Control signals:**
- **Attention:** Both arms raised, crossed overhead
- **Error:** Arms waved crosswise
- **Numerals follow:** Arms crossed below waist
- **End of word:** Right arm lowered to rest
- **End of message:** Both arms crossed below waist
...
| Delta | D | Yellow/blue/yellow | "Keep clear â I am maneuvering with difficulty" |
| Echo | E | Blue/red halves | "I am altering my course to starboard (right)" |
| Foxtrot | F | Red/white diamonds | "I am disabled â communicate with me" |
...
| Lima | L | Black/yellow quarters (diagonal) | "You should stop â I have something important to communicate" |
...
| Papa | P | Blue with white square center | In harbor: "All crew return to ship â about to sail." At sea: "Nets fast on bottom" |
| Quebec | Q | Yellow solid | "My vessel is healthy â request free pratique (permission to land)" |
...
| Tango | T | Red/white/blue vertical | "Keep clear â pair trawling in operation" |
...
| X-ray | X | White with blue cross | "Stop carrying out your intentions â watch for my signals" |
...
Three substitute flags allow repeating any letter in a hoist (since only one of each flag is carried). The number pennant (0â9) and the answering pennant also play roles in numeric signals.
...
## Part 6: Binary â The Language of Machines

### What Is Binary?

Binary is a base-2 number system using only two digits: 0 and 1. All modern computers represent every piece of information â text, images, audio, video â as sequences of 0s and 1s.
...
- Hundreds (10Â²), Tens (10Â¹), Ones (10â°)

In binary (base 2), each position is a power of 2:
- 128 (2â·), 64 (2â¶), 32 (2âµ), 16 (2â´), 8 (2Â³), 4 (2Â²), 2 (2Â¹), 1 (2â°)
...
Example: 13 Ã· 2 = 6 R1, 6 Ã· 2 = 3 R0, 3 Ã· 2 = 1 R1, 1 Ã· 2 = 0 R1 â read remainders upward: **1101** = 13 â
...
* **Byte:** 8 bits â can represent 256 values (0â255)
...
* **Chess:** A 64-square board can represent positions in 6 bits per square (since 2â¶ = 64)

### Binary as Signaling

In theory, any two-state system can transmit binary:
- Light on/off (even a flashlight)
- Flag up/down
- Two distinct sounds (high/low pitch)
- Knocking patterns (one vs. three taps for 0 and 1)

The limitation is speed â humans transmit binary slowly. But for simple coded messages, it works.

---

*Part of The Observatory Almanac â Language & Communication section*
...
<div id="morse-tool" markdown>

<div style="display:flex; gap:16px; flex-wrap:wrap; margin:1em 0;">
<div style="flex:1; min-width:280px;">
<label><strong>Text:</strong></label><br>
<textarea id="morse-text" rows="4" style="width:100%; font-size:14px; padding:8px; border-radius:6px; border:1px solid #888;" placeholder="Type text here..."></textarea><br>
<button onclick="textToMorse()" style="padding:8px 16px; margin-top:4px; cursor:pointer; border-radius:6px; border:1px solid #888; background:#fff;">Text → Morse ▶</button>
</div>
<div style="flex:1; min-width:280px;">
<label><strong>Morse:</strong></label><br>
<textarea id="morse-code" rows="4" style="width:100%; font-size:14px; padding:8px; font-family:monospace; border-radius:6px; border:1px solid #888;" placeholder="Type morse here (use . and - separated by spaces)..."></textarea><br>
<button onclick="morseToText()" style="padding:8px 16px; margin-top:4px; cursor:pointer; border-radius:6px; border:1px solid #888; background:#fff;">◀ Morse → Text</button>
</div>
</div>
...
(function() {
  ...
  const morseToChar = {};
  for (const [k, v] of Object.entries(charToMorse)) morseToChar[v] = k;

  window.textToMorse = function() {
    const text = document.getElementById("morse-text").value.toUpperCase();
    const words = text.split(/\s+/);
    const morse = words.map(w =>
      [...w].map(c => charToMorse[c] || c).join(' ')
    ).join('   /   ');
    document.getElementById("morse-code").value = morse;
  };

  window.morseToText = function() {
    const morse = document.getElementById("morse-code").value.trim();
    const words = morse.split(/\s*\/\s*/);
    const text = words.map(w =>
      w.split(/\s+/).map(c => morseToChar[c] || c).join('')
    ).join(' ');
    document.getElementById("morse-text").value = text;
  };
})();
</script>
