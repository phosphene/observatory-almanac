...
Your router probably broadcasts two different WiFi signals. You'll see them in your device's WiFi list, often named something like "HomeNetwork" and "HomeNetwork\_5G" — or identical names with different signals if your router is newer.
...
2. **192.168.1.1** — Try this first
3. **192.168.0.1** — Second most common
4. **10.0.0.1** — Used by Apple and some others
   The correct address is sometimes printed on the router sticker.
5. Log in with the admin credentials (also on the sticker — default is often "admin" / "admin" or "admin" / "password")
6. Look for "Wireless Settings" or "WiFi Settings"
7. Change the SSID (network name) to something you'll recognize — avoid using your address or full name
...
- Rogue access points — fake networks named "Airport\_Free\_WiFi" designed to intercept your data
