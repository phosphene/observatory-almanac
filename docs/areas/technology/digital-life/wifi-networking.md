# WiFi & Home Networking â Plain English Guide

WiFi is one of those things that works invisibly when it's working and completely dominates your day when it isn't. This guide explains what's actually going on, how to improve it, and how to fix the most common problems â without needing a degree in networking.
...
**Modem:** The box that connects your home to the internet. It talks to your ISP (Internet Service Provider â your cable company or phone company) and brings the internet signal into your house. It usually has a coaxial cable (like a TV cable) or a phone line going into it, and an ethernet port coming out.

**Router:** The box that takes the internet from the modem and shares it to all your devices â phones, laptops, TVs, and everything else. It also creates your WiFi network. It typically has several ethernet ports plus an antenna or two.

**Gateway:** One device that does both jobs â modem and router combined. This is what most ISPs provide when you sign up for service. When someone says "restart your router," they usually mean this box.

If you have two boxes, one is the modem and one is the router. If you have one box, it's a gateway. Either setup works.

**The chain:** Internet â ISP â cable/phone line â modem (or gateway) â router â WiFi â your devices

### 2.4 GHz vs. 5 GHz

Your router probably broadcasts two different WiFi signals. You'll see them in your device's WiFi list, often named something like "HomeNetwork" and "HomeNetwork\_5G" â or identical names with different signals if your router is newer.

**2.4 GHz:**
- Longer range â reaches farther through walls and floors
- Slower top speed
...
**5 GHz:**
- Shorter range â doesn't penetrate walls as well
- Much faster speeds when you're nearby
- Less congested (fewer neighbors use it)
- Better for: video streaming, video calls, gaming â when you're in the same room or next room as the router
...
These are WiFi generations â the technical standards that determine speed and efficiency.
...
**Bottom line:** If your router is over 5 years old and you're frustrated with WiFi, upgrading to a WiFi 6 router will likely help â especially if you have 10+ connected devices. If your router is newer, you almost certainly don't need to upgrade.
...
**On an iPhone already connected:** Settings â WiFi â tap the connected network name â look for password (iOS 16+) or tap the info icon. In iOS 16 and later, there's a visible password field you can tap to reveal.

**On an Android already connected:** Settings â WiFi â tap the connected network â "Share" button generates a QR code. Or Settings â WiFi â tap the network â you may see the password directly on some Android versions.

**On a Windows computer already connected:** Control Panel â Network and Sharing Center â click your WiFi network name â Wireless Properties â Security tab â check "Show Characters" next to the password field.

**On a Mac:** Use Keychain Access (search for it in Spotlight) â search for your network name â double-click â check "Show password" â enter your Mac login password.
...
**Ideal placement:**
- **Central in your home** â WiFi radiates outward in all directions; a router in the corner only covers three directions effectively
- **Elevated** â On a shelf or desk, not on the floor. Signals travel horizontally and slightly downward.
- **Open space** â Not inside a cabinet, closet, or entertainment center. Every physical barrier degrades the signal.
...
**The garage test:** If your WiFi works fine everywhere but the back bedroom or the back yard, you have a range problem â not a speed problem. Move the router or add an extender.

### Changing Your WiFi Name and Password

You should do this when you first set up a router. Using the default network name ("NETGEAR\_5G" or "Linksys00437") broadcasts that you're using default settings, which can hint at easier-to-guess admin passwords.

1. Type your router's IP address into a web browser. Common addresses:
2. **192.168.1.1** â Try this first
3. **192.168.0.1** â Second most common
4. **10.0.0.1** â Used by Apple and some others
   The correct address is sometimes printed on the router sticker.
5. Log in with the admin credentials (also on the sticker â default is often "admin" / "admin" or "admin" / "password")
6. Look for "Wireless Settings" or "WiFi Settings"
7. Change the SSID (network name) to something you'll recognize â avoid using your address or full name
8. Change the password to something strong: 12+ characters, mix of letters and numbers
...
**Why you want this:**
1. Give visitors WiFi without sharing your main network password â guests get internet access but can't see your computers and other devices
2. Put IoT (Internet of Things) devices on it: smart TVs, smart speakers, doorbells, thermostats. These devices often have poor security â keeping them on a separate network means a hacked thermostat can't reach your laptop.

**How to set it up:** Router admin page â look for "Guest Network" â enable it â give it a name and separate password.

### Connecting Devices That Struggle

**Smart TVs and streaming sticks:**
- Connect to 5 GHz if within 20 feet of the router; 2.4 GHz if farther
- Some smart TVs have poor WiFi antennas â if you can run an ethernet cable, that's faster and more reliable

**Printers:**
- Most struggle with 5 GHz â put them on 2.4 GHz
...
**Fix it:**
1. Log into your router admin page (see above â 192.168.1.1 or similar)
2. Find "Administration" or "Settings" â "Router Password" or "Admin Password"
3. Change it to something unique and strong â not your WiFi password
...
**How to check:** Router admin page â Wireless Settings â Security Mode. Set to WPA3 if available, WPA2-AES if not. Avoid "TKIP" â it's weaker.
...
**Fix:** Log into router admin â Wireless Settings â find WPS â Disable.
...
- Any device you can't identify â investigate. It might be a neighbor using your network, or it might be a smart home device you forgot about.
...
- Rogue access points â fake networks named "Airport\_Free\_WiFi" designed to intercept your data
...
- Don't do anything you'd be uncomfortable with the cafÃ© owner watching

**When to use a VPN:** If you regularly use public WiFi and want to be safe: a VPN encrypts all your traffic, even on unencrypted networks. See [Laptop Setup](../laptop-setup/) for VPN recommendations.

---

## Troubleshooting

### "Connected But No Internet" â The 5-Step Fix
...
- iPhone: Settings â WiFi â tap the network â "Forget This Network" â reconnect
- Android: Settings â WiFi â tap and hold the network â Forget â reconnect
...
**Step 4: Test close to the router.** Put your phone right next to the router and test. If it's much faster than in the room you normally use it, it's a range problem â see extenders and mesh systems above.

**Common causes of slow WiFi:**
- **Channel congestion:** If all your neighbors are on the same WiFi channel, you're competing for bandwidth. Router admin â Wireless Settings â Channel. Try setting it manually to channels 1, 6, or 11 (for 2.4 GHz). Apps like WiFi Analyzer (Android) show which channels are crowded.
...
**iPhone:** Settings â WiFi â tap the (i) next to the network â "Forget This Network" â confirm
**Android:** Settings â WiFi â tap and hold the network name â "Forget"
**Windows:** Click the WiFi icon in the taskbar â click the arrow next to the network â "Forget"
**Mac:** System Settings â WiFi â click the network â remove (minus button)
...
**When calling your ISP:** Note the light status on your router (write down which lights are on/off/blinking). Have your account number ready. Ask specifically: "Is there an outage in my area?" â this tells you immediately if it's their problem or yours.

---

*See also: [Laptop Setup](../laptop-setup/) | [Smartphone Setup](../smartphone-setup/) | [Streaming & Entertainment](../streaming-entertainment/)*